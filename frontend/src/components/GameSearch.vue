<template>
  <div class="relative w-full">
    <input
      ...
      class="w-full px-3 py-2 sm:px-4 sm:py-3 pr-10 sm:pr-12 text-base sm:text-lg border border-gray-600 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-gray-700 text-white placeholder-gray-400"
    ...
    <button
      ...
      @click="clearSearch"
      class="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-white transition-colors p-1 rounded hover:bg-gray-600"
      title="Clear search"
    >
    ...
    <div
      ...
      class="absolute z-10 w-full mt-2 bg-gray-800 border border-gray-600 rounded-lg shadow-2xl max-h-80 sm:max-h-96 overflow-y-auto"
    >
      <!-- Loading State -->
      <div v-if="isSearching" class="p-3 sm:p-4 text-center text-gray-400 text-sm sm:text-base">
        Searching...
      </div>
      ...
      <div
        ...
        class="p-3 sm:p-4 text-center text-gray-400 text-sm sm:text-base border-b border-gray-700"
      >
        No results found
      </div>
      ...
      <div
        ...
        class="p-2 sm:p-3 hover:bg-gray-700 cursor-pointer border-t-2 border-gray-600 transition-colors"
        @click="createManual"
      >
        <div class="flex items-center gap-2">
          ...
          <span class="font-semibold text-blue-400 text-sm sm:text-base truncate">Manually create an entry for "{{ searchText }}"</span>
        </div>
      </div>
    </div>
  </div>
