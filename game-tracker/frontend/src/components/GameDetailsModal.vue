...
const emit = defineEmits(['close', 'update-status', 'delete-game'])
...
function handleMatchUpdated(updatedGame) {
  // The game prop will be updated by the parent via computed property
  // Just close the Fix Match modal
  isFixMatchModalOpen.value = false
}
...
function startEditingDate() {
  ...
  // Set current date or today as default
...
function saveDateEdit() {
  if (editedDate.value) {
    // Convert date string to ISO 8601 datetime string
    const dateObj = new Date(editedDate.value + 'T12:00:00Z')
    const isoDateString = dateObj.toISOString()

    // Emit update-status with the current status and new date
    emit('update-status', props.game.id, props.game.status, isoDateString)
  }
  // Exit edit mode - the parent will update the game prop with fresh data
  isEditingDate.value = false
  editedDate.value = null
}
